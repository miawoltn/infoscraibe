...
const MessageList = ({
  ...
  const handleDeleteVersion = (messageId: string, versionIndex: number) => {
    ...
    return {
      title: `Delete ${isCurrentVersion ? "Current" : `Version ${versionNumber}`
        }`,
      description: `Are you sure you want to delete ${isCurrentVersion ? "the current version" : `version ${versionNumber}`
        }? ${totalVersions <= 2
          ? "This will revert to the original response."
          : `You will still have ${totalVersions - 1} version${totalVersions - 1 > 1 ? "s" : ""
          }.`
        }`,
