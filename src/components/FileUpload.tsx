...
const FileUpload = ({
    ...
    const { mutate, isPending, reset: createReset } = useMutation({
        mutationFn: async ({ fileKey, fileName, fileType, checksum }: { fileKey: string, fileName: string, fileType: string, checksum: string }) => {
            const { data } = await axios.post('/api/chat/create', { fileKey, fileName, fileType, checksum })
            return data
        }
    })
    const { mutate: deleteChat, isPending: isDeletingChat, reset } = useMutation({
        mutationFn: async ({ fileKey }: { fileKey: string }) => {
            const { data } = await axios.delete(`/api/chat/${encodeURIComponent(fileKey)}`)
            return data
        }
    })
        ...
        <>
                ...
                <DialogContent>
                    <div className="w-full">
                        <div className='p-2 bg-white dark:bg-slate-900 rounded-xl'>
                            <div {...getRootProps({
                                className: cn('border border-dashed border-gray-300 rounded-xl bg-gray-50 dark:bg-slate-900 py-8 px-8 flex justify-center items-center flex-col w-full h-full', {
                                ...
                                {isPending || uploading ?
                                    (
                                        <>
                                            <Progress
                                                value={progress}
                                                className={cn("w-[100%] h-2 mt-1 mb-1 bg-zinc-200 dark:bg-slate-900", {
                                    ...
                                    (<>
                                        ...
                                        <p className='text-xs text-zinc-500'>
                                            {Object.values(fileTypes).flat().join(',')} (up to {fileSize}MB)
                                        </p>
                                    </>)
                                }
                            </div>
            ...
            <Dialog open={showAlert} onOpenChange={(b) => { console.log(b) }}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        <Button variant='outline' onClick={async () => { await cancleOngoingUplaod(false) }}>No</Button>
                        <Button variant='destructive' onClick={async () => await cancleOngoingUplaod(true)}>Yes</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </>
